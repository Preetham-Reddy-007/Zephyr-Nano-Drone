...
// #include "trace.h" ----- TRACING HAS BEEN ENABLED (INSTEAD OF ITM TRACING WHIHC IS NOT IN SUPPORT) 
//                          ZEPHYR RTOS REGULAR TRACING IS ENBLED IN CONFIGURATION FILE
// #include "usec_time.h" ---- NOT NEEDED EXPLAINED BELOW
/*
There's a description of how to do something like that here.

Essentially you can use the k_cycle_get_32() to get an accurate timestamp from Zephyr, 
and if you divide this number by 32768 (the freq. of the LF clock) you will get the number of seconds passed. 
Then you need to implement this as part of the data you transmit over BLE. Likely in a service.

Creating a microsecond-resolution timestamp on the nRF5340 involves using the Zephyr RTOS's `k_cycle_get_32()` function¹. 
This function returns an accurate timestamp, and if you divide this number by 32768 (the frequency of the Low Frequency clock), you will get the number of seconds passed¹. 
You can then multiply this value by 1,000,000 to convert it to microseconds. 
...
uint64_t get_microsecond_timestamp(void)
{
    ...
    uint64_t microseconds = (uint64_t)cycles * 1000000 / CONFIG_SYS_CLOCK_HW_CYCLES_PER_SEC;
    return microseconds;
}
```

This function will return a timestamp in microseconds. Please note that the actual resolution depends on the system clock frequency. 
If it's less than 1 MHz, the resolution will be lower than 1 microsecond.
Remember to include the Zephyr header file (`#include <zephyr.h>`) and to set the system clock frequency (`CONFIG_SYS_CLOCK_HW_CYCLES_PER_SEC`) correctly in your project configuration¹.

Source: Conversation with Bing, 21/8/2023
(1) Timestamp Data in nRF5340 - Nordic Q&A - Nordic DevZone. https://devzone.nordicsemi.com/f/nordic-q-a/84079/timestamp-data-in-nrf5340.
(2) timer in microsecond on nRF52840 - Nordic Q&A - Nordic DevZone. https://devzone.nordicsemi.com/f/nordic-q-a/37896/timer-in-microsecond-on-nrf52840.
*/
...
// -------Task stack sizes----------
/*
The configMINIMAL_STACK_SIZE - is defined in FreeRTOS as stack size for idle thread
We have to set configMINIMAL_STACK_SIZE - For our application
We are setting the configMINIMAL_STACK_SIZE as 1024 right now below
*/
