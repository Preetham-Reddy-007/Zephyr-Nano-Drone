...
struct k_thread platformSrvTask;
K_THREAD_STACK_DEFINE(platformSrvTaskStack, PLATFORM_SRV_TASK_STACKSIZE);
...
static void platformSrvTaskFunction(void *a, void *b, void *c);
...
void platformserviceInit(void)
{
  ...
  k_thread_create(&platformSrvTask, platformSrvTaskStack,
                                 K_THREAD_STACK_SIZEOF(platformSrvTaskStack),
                                 platformSrvTaskFunction,
                                 NULL, NULL, NULL,
                                 PLATFORM_SRV_TASK_PRI, 0, K_NO_WAIT);
  isInit = true;
}
...
static void platformSrvTaskFunction(void* prm)
{
  static CRTPPacket p;
